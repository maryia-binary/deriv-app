...
import { TBottomWidgetsParams } from 'Modules/Trading/Containers/trade';
...
type TToolbarWidgetProps = React.PropsWithChildren<{ position?: string }>;
...
type TFastMarkerProps = React.PropsWithChildren<{
...
type TDerivChartComponents = {
    SmartChart: React.ComponentType<TSmartChartProps>;
    ChartTitle: React.ComponentType<TChartTitleProps>;
    ChartSize: React.ComponentType<Record<string, never>>;
    ChartMode: React.ComponentType<TChartModeProps>;
    DrawTools: React.ComponentType<TPropsWithPortalNodeId>;
    Share: React.ComponentType<TPropsWithPortalNodeId>;
    StudyLegend: React.ComponentType<TPropsWithPortalNodeId>;
    Views: React.ComponentType<TPropsWithPortalNodeId>;
    ToolbarWidget: React.ComponentType<TToolbarWidgetProps>;
    FastMarker: React.ComponentType<TFastMarkerProps>;
};

type TDerivChartsModule = TDerivChartComponents & {
...
const init = () => {
    module = moduleLoader(() => {
        return import(/* webpackChunkName: "smart_chart" */ '@deriv/deriv-charts' as string);
    }) as Promise<TDerivChartsModule>;
...
const load = (component_name: keyof TDerivChartComponents) => () => {
    if (!module) {
        init();
    }
